...
public class DemoDataSeed implements ApplicationRunner {
 ...
 public void run(ApplicationArguments ignored){
   ...
   for(int i=1;i<=8;i++){final int n=i;UUID id=id("user-"+n); AppUser u=users.findById(id).orElseGet(()->users.save(new AppUser(id,"demo"+n+"@local.invalid",demoUsername(n),encoder.encode("demo-only-password"),Role.USER,base)));demoUsers.add(u);}
 ...
 static String demoUsername(int number){return "demo"+number;}
