...
@Service public class GamificationService {
 ...
 public void enforceCooldown(UUID user,String identity,long millis){Instant at=Instant.ofEpochMilli(millis);if(events.cooldown(user,identity,at.minus(Duration.ofMinutes(30)),at))throw new ApiException(HttpStatus.CONFLICT,"feedback_cooldown","Only one feedback per line is allowed every 30 minutes");}
