...
@Service public class GamificationService {
 ...
 public void revoke(AppUser user,UUID feedbackId){UUID userId=user.getId();if(events.existsByUserIdAndTypeAndSourceKey(userId,"FEEDBACK_BASE_AWARDED",feedbackId.toString()))add(user,"FEEDBACK_BASE_REVOKED",feedbackId.toString(),-BASE_XP,null,Instant.now());evaluate(user,Instant.now());}
 private int add(AppUser u,String type,String source,int xp,String identity,Instant at){if(events.existsByUserIdAndTypeAndSourceKey(u.getId(),type,source))return 0;events.saveAndFlush(new GamificationEvent(u,type,source,xp,identity,at));return xp;}
 public List<String> evaluate(AppUser user,Instant at){UUID userId=user.getId();var rows=feedback.findByOwnerId(userId);long count=rows.size(),lines=rows.stream().map(f->f.transportType+":"+f.normalizedLine).distinct().count(),types=rows.stream().map(f->f.transportType).distinct().count(),days=rows.stream().map(f->Instant.ofEpochMilli(f.createdAt).atZone(ZoneOffset.UTC).toLocalDate()).distinct().count();List<String> fresh=new ArrayList<>();
  for(var d:AchievementCatalog.ALL){long p=switch(d.kind()){case CONTRIBUTIONS->count;case LINES->lines;case TYPES->types;case ACTIVE_DAYS->days;case TYPE_COUNT->rows.stream().filter(f->f.transportType.name().equals(d.type())).count();};if(p>=d.target()&&!achievements.existsByUserIdAndCode(userId,d.code())){achievements.save(new UserAchievement(user,d.code(),at));fresh.add(d.code());}} return fresh;}
 public void backfillAchievements(AppUser user){UUID userId=user.getId();var rows=feedback.findByOwnerId(userId).stream().sorted(Comparator.comparingLong(f->f.createdAt)).toList();Set<String> lines=new HashSet<>(),types=new HashSet<>();Set<java.time.LocalDate> days=new HashSet<>();Map<String,Long> typeCounts=new HashMap<>();long contributions=0;
  for(var row:rows){contributions++;lines.add(row.transportType+":"+row.normalizedLine);types.add(row.transportType.name());days.add(Instant.ofEpochMilli(row.createdAt).atZone(ZoneOffset.UTC).toLocalDate());typeCounts.merge(row.transportType.name(),1L,Long::sum);Instant reached=Instant.ofEpochMilli(row.createdAt);
   for(var d:AchievementCatalog.ALL){long p=switch(d.kind()){case CONTRIBUTIONS->contributions;case LINES->lines.size();case TYPES->types.size();case ACTIVE_DAYS->days.size();case TYPE_COUNT->typeCounts.getOrDefault(d.type(),0L);};if(p>=d.target()){var existing=achievements.findByUserIdAndCode(userId,d.code());if(existing.isEmpty())achievements.save(new UserAchievement(user,d.code(),reached));else if(existing.get().unlockedAt.isAfter(reached))existing.get().unlockedAt=reached;}}
  }}
}
