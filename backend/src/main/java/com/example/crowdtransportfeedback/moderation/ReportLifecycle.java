...
import com.example.crowdtransportfeedback.gamification.GamificationService; import com.example.crowdtransportfeedback.user.*; import java.time.Instant; import java.util.UUID; import org.springframework.stereotype.Service;
@Service public class ReportLifecycle {
 ...
 public int resolve(UUID feedbackId,UUID adminId,boolean confirmed,String note){var pending=reports.findByFeedbackIdAndStatus(feedbackId,ReportStatus.PENDING);Instant now=Instant.now();var admin=users.getReferenceById(adminId);for(var r:pending){r.status=confirmed?ReportStatus.CONFIRMED:ReportStatus.DISMISSED;r.resolvedAt=now;r.resolvedBy=admin;}reports.saveAll(pending);if(confirmed)pending.stream().map(r->r.reporter).distinct().forEach(u->gamification.evaluate(u,now));audit.save(new AdminAuditLog(admin,confirmed?"REPORTED_FEEDBACK_DELETED":"REPORTS_DISMISSED",feedbackId,note));return pending.size();}
 public void close(UUID feedbackId){var pending=reports.findByFeedbackIdAndStatus(feedbackId,ReportStatus.PENDING);Instant now=Instant.now();pending.forEach(r->{r.status=ReportStatus.CLOSED;r.resolvedAt=now;});reports.saveAll(pending);}
 ...
 public void auditDelete(UUID feedbackId,UUID adminId){audit.save(new AdminAuditLog(users.getReferenceById(adminId),"FEEDBACK_DELETED",feedbackId,null));}
}
