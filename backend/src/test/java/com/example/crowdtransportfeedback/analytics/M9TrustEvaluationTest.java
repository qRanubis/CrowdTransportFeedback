...
class M9TrustEvaluationTest {
    ...
    @Test void m9TrustExperiments() {
        ...
        for (int i=0;i<20;i++) e1.add(row(i, i%2==0?3:4, 0));
        ...
        var e2=new ArrayList<Feedback>(); for(int i=0;i<20;i++)e2.add(row(i,4,0));
        for(int i=0;i<20;i++)e2.add(row(100,1,0));
        ...
        for(int age:new int[]{30,60,90}) {
            ...
            for(int i=0;i<10;i++) rows.add(row(i,2,age));
            for(int i=10;i<20;i++) rows.add(row(i,4,0));
            emit("E3","age_"+age,"age_days",age); errors("E3","age_"+age,4,rows);
            assertTrue(Math.abs(proposed(rows)-4)<Math.abs(mean(rows)-4));
        }

        var e4=List.of(row(0,5,0)); b=mean(e4);p=proposed(e4);
        ...
        for(int i=0;i<20;i++){repeated.add(row(0,5,0));diverse.add(row(i,5,0));}
        ...
        for(int j=0;j<counts.length;j++){var rows=new ArrayList<Feedback>();for(int i=0;i<counts[j];i++)rows.add(row(i,4,0));
    ...
    private static Feedback row(int contributor,int score,int ageDays){UUID uid=new UUID(0,contributor+1L);AppUser u=new AppUser(uid,"m9"+contributor+"@example.test","m9u"+contributor,"x",Role.USER,NOW.minusSeconds(86400L*ageDays));return new Feedback(new UUID(contributor+1L,score),u,TransportType.METRO,"M9EVAL",score,score,score,"",44.4268,26.1025,NOW.minusSeconds(86400L*ageDays).toEpochMilli());}
}
