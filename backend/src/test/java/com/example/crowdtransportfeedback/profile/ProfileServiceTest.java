...
class ProfileServiceTest {
    ...
    @Test void fullCatalogOrderNeverDependsOnPins(){var ordered=ProfileService.orderedDefinitions(List.of("METRO_EXPLORER"),false);assertEquals("Contribution",ordered.getFirst().category());assertEquals(List.of("Contribution","Network Exploration","Transport","Consistency"),ordered.stream().map(AchievementCatalog.Definition::category).distinct().toList());}
    ...
    @Test void unlockedProgressNeverFallsBelowTargetWhileLockedProgressRemainsLive(){assertEquals(1,ProfileService.displayProgress(0,1,true));assertEquals(0,ProfileService.displayProgress(0,1,false));assertEquals(25,ProfileService.displayProgress(7,25,true));}
}
