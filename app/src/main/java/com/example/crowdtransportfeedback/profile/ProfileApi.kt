...
data class PinnedAchievementsRequest(val achievementCodes: List<String>)
interface ProfileApi {
 ...
 @PUT("api/profile/me/pinned-achievements") suspend fun pins(@Body body:PinnedAchievementsRequest): Response<Unit>
 @GET("api/leaderboard") suspend fun leaderboard(@Query("metric") metric:String,@Query("period") period:String):LeaderboardDto
}

suspend fun ProfileApi.updatePins(codes: List<String>) {
 val response = pins(PinnedAchievementsRequest(codes))
