...
data class ModerationDetail(val feedback:Map<String,Any?>,val reports:List<PendingReport>)
...
interface AdminApi {
 ...
 @GET("api/admin/feedback") suspend fun feedback(@Query("window")window:String="ALL",@Query("page")page:Int=0,@Query("size")size:Int=20):AdminPage<AdminFeedback>
 ...
 @GET("api/admin/reporting/summary") suspend fun summary(@Query("window")window:String="ALL"):ReportingSummary
 @GET("api/admin/reporting/feedback.csv") suspend fun csv(@Query("window")window:String="ALL"):ResponseBody
}
