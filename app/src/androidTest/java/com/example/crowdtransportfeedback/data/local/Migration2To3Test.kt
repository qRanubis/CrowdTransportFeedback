...
class Migration2To3Test {
 ...
 @Test fun legacyRowSurvivesWithNullStructuredFields() = runBlocking {
  ...
  val database=Room.databaseBuilder(context,AppDatabase::class.java,name).addMigrations(MIGRATION_2_3).allowMainThreadQueries().build()
  val row=database.feedbackDao().getByLocalIdOnce(7)!!
  assertEquals(7,row.localId); assertEquals("stable-uuid",row.feedbackId); assertEquals(5,row.score); assertEquals(SyncState.SYNCED,row.syncState)
  assertNull(row.transportType); assertNull(row.crowdingScore); assertNull(row.cleanlinessScore); assertNull(row.punctualityScore)
  database.close()
 }
}
