param([int[]]$DatasetSizes=@(100,1000,5000,10000),[int]$Warmups=5,[int]$Iterations=30,[int]$BackendPort=8081,[string]$EvaluationDatabase='crowd_feedback_eval')
...
$dbUser=if($config['DATABASE_USERNAME']){$config['DATABASE_USERNAME']}else{'crowd'}
$adminUsername=if($config['APP_ADMIN_USERNAME']){$config['APP_ADMIN_USERNAME']}else{'admin'}
$container='crowdtransportfeedback-m9-backend';$base="http://localhost:$BackendPort";$resultsDir=Join-Path $PSScriptRoot 'results';New-Item -ItemType Directory -Force $resultsDir|Out-Null
...
function Request([string]$Name,[string]$Uri,[string]$Token){
 try{return Invoke-RestMethod -Uri $Uri -Headers @{Authorization="Bearer $Token"} -Method Get}
  ...
  $status='unknown';if($_.Exception.Response -and $_.Exception.Response.StatusCode){$status=[int]$_.Exception.Response.StatusCode}
  $body=$_.ErrorDetails.Message;if(!$body){$body='unavailable'}
  throw "HTTP request failed for '$Name' (status $status). URI: $Uri. Response: $body"
 }
}
...
function Timed([string]$Name,[string]$Uri,[string]$Token,[int]$Size){for($i=0;$i -lt $Warmups;$i++){[void](Request $Name $Uri $Token)};$times=[System.Collections.Generic.List[double]]::new();$failures=0;for($i=0;$i -lt $Iterations;$i++){$sw=[Diagnostics.Stopwatch]::StartNew();try{[void](Request $Name $Uri $Token);$sw.Stop();$times.Add($sw.Elapsed.TotalMilliseconds)}catch{$sw.Stop();$failures++;throw "Measured request failed for $Name; dataset aborted and no misleading row written: $($_.Exception.Message)"}};$sorted=@($times|Sort-Object);function NR([double]$p){$sorted[[Math]::Max(0,[Math]::Ceiling($p*$sorted.Count)-1)]};[pscustomobject]@{dataset_size=$Size;endpoint=$Name;warmup_count=$Warmups;iteration_count=$Iterations;success_count=$times.Count;failure_count=$failures;min_ms=[Math]::Round($sorted[0],3);average_ms=[Math]::Round(($times|Measure-Object -Average).Average,3);p50_ms=[Math]::Round((NR .50),3);p95_ms=[Math]::Round((NR .95),3);max_ms=[Math]::Round($sorted[-1],3)}}
...
try{
 ...
 foreach($size in $DatasetSizes){Psql $EvaluationDatabase (Get-Content (Join-Path $PSScriptRoot 'sql/reset-performance-data.sql') -Raw) @('-v',"evaluation_db=$EvaluationDatabase");Psql $EvaluationDatabase (Get-Content (Join-Path $PSScriptRoot 'sql/seed-performance-data.sql') -Raw) @('-v',"row_count=$size")
  $heatUri="$base/api/analytics/heatmap?metric=TRUST&transportType=METRO&line=M9EVAL&window=ALL";$heatResponse=Request 'analytics_heatmap' $heatUri $token;$heatCells=AsArray $heatResponse;if($heatCells.Count -eq 0){throw "Correctness failure: empty heatmap for $size (logical cells received: 0)"};$firstCell=$heatCells[0];$cell=[string]$firstCell.cellId;if($cell -notmatch '^-?\d+:-?\d+$'){throw "Correctness failure: malformed heatmap cellId '$cell' for $size (logical cells received: $($heatCells.Count))"};$areaUri="$base/api/analytics/area?cellId=$cell&metric=TRUST&transportType=METRO&line=M9EVAL&window=ALL"
  $overview=Request 'admin_overview' "$base/api/admin/overview" $token;if([long]$overview.totalFeedbacks -ne $size){throw "Correctness failure: overview totalFeedbacks != $size"};$summaryUri="$base/api/admin/reporting/summary?window=ALL&transportType=METRO&line=M9EVAL";$summary=Request 'admin_reporting_summary' $summaryUri $token;if([long]$summary.feedbackCount -ne $size){throw "Correctness failure: summary feedbackCount != $size"};$adminFeedbackUri="$base/api/admin/feedback?transportType=METRO&line=M9EVAL&window=ALL&page=0&size=20";$adminRows=Request 'admin_feedback' $adminFeedbackUri $token;$adminFeedbackItems=AsArray $adminRows.content;if([long]$adminRows.totalElements -ne $size -or $adminFeedbackItems.Count -eq 0){throw 'Correctness failure: filtered admin feedback is incomplete'};[void](Request 'analytics_area' $areaUri $token)
